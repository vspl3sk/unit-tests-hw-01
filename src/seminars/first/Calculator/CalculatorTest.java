...
//        ).isInstanceOf(IllegalStateException.class);
        ...
        testCalculateDiscountWithInvalidDiscountPercent();
        testCalculateDiscountWithNegativeDiscountPercent();
...
// Ваша задача - проверить этот метод с использованием библиотеки AssertJ. Если метод calculateDiscount получает недопустимые аргументы, он должен выбрасывать исключение ArithmeticException. Не забудьте написать тесты для проверки этого поведения.
    public static void testCalculateDiscountWithValidArguments() {
        ...
        double result = calculator.calculateDiscount(1000.0, 20.0);
    ...
    public static void testCalculateDiscountWithNegativePurchaseAmount() {
        ...
        assertThatThrownBy(() -> calculator.calculateDiscount(-1000.0, 30.0))
                .isInstanceOf(ArithmeticException.class)
                .hasMessage("Invalid arguments");
    }

    public static void testCalculateDiscountWithNegativeDiscountPercent() {
        Calculator calculator = new Calculator();

        assertThatThrownBy(() -> calculator.calculateDiscount(1000.0, -25.0))
                .isInstanceOf(ArithmeticException.class)
                .hasMessage("Invalid arguments");
    }

    public static void testCalculateDiscountWithInvalidDiscountPercent() {
        Calculator calculator = new Calculator();

        assertThatThrownBy(() -> calculator.calculateDiscount(1000.0, 150.0))
                .isInstanceOf(ArithmeticException.class)
                .hasMessage("Invalid arguments");
    }
}